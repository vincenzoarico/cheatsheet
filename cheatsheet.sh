...
execute() {
    ...
    # Tealdeer
    tealdeer_output=$(tldr "$(sanitize_name "$1")" 2>/dev/null)
    if [[ -n "$tealdeer_output" ]]; then
        echo -e "${GREEN}${SEPARATOR}TEALDEER${SEPARATOR}${RESET}"
        echo "$tealdeer_output"
    else
	echo -e "${RED}Command not found in tldr${RESET}"
    fi
    ...
    if [[ -n "$cheat_output" ]]; then
        echo -e "${BLUE}${SEPARATOR}CHEAT.SH${SEPARATOR}${RESET}"
        echo "$cheat_output"
    else
	echo -e "${RED}Failed to fetch from cheat.sh${RESET}"
    fi
    ...
    if [[ -n "$custom_output" ]]; then
        echo -e "${YELLOW}${SEPARATOR}YOUR COMMANDS${SEPARATOR}${RESET}"
        echo "$custom_output"
    else
	echo -e "${RED}No personal commands found${RESET}"
    fi
...
help(){
    ...
    echo "CHEAT: cheatsheet execute <cmd_type>"
    echo "HELP: cheatsheet help"

    echo ""
    echo "Each command parameter has to be passed between \"parameter\" if contains spaces"
    echo "Path to manage your added commands: '$CHEATSHEET_COMMANDS'"
